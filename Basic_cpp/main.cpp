...
/*Дано число N (0<N<10000).
Найти максимальное четное число меньшее N,
сумма цифр которого является простым числом */
...
int zadacha_13()
{
    ...
    cout << "Enter number: ";
    cin >> num;
    if((num-1)%2 == 0) i = num-1; // находим ближайшее четное число, будет или n-1
    else i = num-2; // или n-2
    for(; i>1; i-=2) //тут цикл с шагом -2, потому что нужны только четные
    {
        sum = 0;
        num = i; // проверяемое четное пихаем во временную переменную
        do sum += (num % 10); // суммируем остаток
        while(num /= 10); // пока истина, продолжаем цикл суммирования
        for(j=2; j*j<=sum; j++) // цикл проверки sum на простое число
            if(sum%j == 0) break; // если остаток станет 0 то выходим из проверки, иначе проверяем дальше
        if(j*j > sum) break; // если проверка пройдена до конца, то выходим из цикла, sum - простое число
    }
    cout << i;
    return  0;
}
